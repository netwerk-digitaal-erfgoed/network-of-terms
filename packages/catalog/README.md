...
- A place or nationality is an **entity** the source mentions. Construct the source’s own **URI**
  where it has one, and name it with `schema:name` or `skos:prefLabel` as for
  `schema:additionalType` above. Where the source only has a name, construct the literal as the
  object itself; the API returns it as an entity with a name and no URI. Each object is one
  entity, so construct one per thing the source mentions, and where a source states several
  places, they are read as alternatives it could not decide between. RKDartists states a place
  or nationality as a URI beside Dutch and English literals with nothing pairing them, so its
  queries name the URI with the literals only when the person has one, and construct the
  literals as entities of their own otherwise.
