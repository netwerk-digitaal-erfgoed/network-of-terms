export const schema = (languages: string[]) => `
  ...
  type Person {
    ...
    "Where the person was born: the place as a term where the source identifies it, and as a name where it only names it. Several places are alternatives the source could not decide between, as when the literature disagrees on a painter’s birthplace; they are not several places."
    birthPlace: Referents!

    "Where the person died, as for \`birthPlace\`."
    deathPlace: Referents!
    ...
    "The person’s nationality, as a term or as a name, as for \`birthPlace\`."
    nationality: Referents!
  }
  ...
  """
  What a source states for a property: the terms it refers to, and the names it gives as text without identifying what they name. A source with a vocabulary of its own fills \`term\`; one that only knows a name fills \`name\`; RKDartists fills both for a nationality, with nothing pairing the two.
  """
  type Referents {
    term: [Reference]!
    name: [LanguageString]!
